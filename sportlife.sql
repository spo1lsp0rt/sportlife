...
--
-- Хост: 127.0.0.1:3306
-- Время создания: Окт 24 2022 г., 10:24
...
-- Процедуры
--
CREATE DEFINER=`root`@`127.0.0.1` PROCEDURE `getAllStatFromOfp` (IN `gender` TEXT, IN `studentFrom` TEXT, IN `yearTest` TEXT)  BEGIN
...
CREATE DEFINER=`root`@`127.0.0.1` PROCEDURE `getNormativesForTest4` (IN `gender` TEXT)  BEGIN
...
CREATE DEFINER=`root`@`127.0.0.1` PROCEDURE `getStatistic` (IN `idClass` INT)  SELECT * FROM `statistic` WHERE `ID_User` IN (select ID_User FROM user WHERE id_class = idClass)$$
...
CREATE TABLE `ofp_normatives` (
  `id` int NOT NULL,
  `name` text NOT NULL,
  `male_normative` text CHARACTER SET utf8 COLLATE utf8_general_ci,
  `female_normative` text CHARACTER SET utf8 COLLATE utf8_general_ci,
  `unit` text NOT NULL,
  `img` text NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3;
