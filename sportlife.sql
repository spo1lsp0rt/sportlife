...
--
-- Хост: 127.0.0.1:3306
-- Время создания: Мар 09 2023 г., 14:55
...
CREATE DEFINER=`root`@`%` PROCEDURE `getAllStatFromTest` (IN `gender` TEXT, IN `studentFrom` TEXT, IN `numTest` INT, IN `yearTest` TEXT)   BEGIN
IF(studentFrom = "Все факультеты") THEN
...
CREATE DEFINER=`root`@`%` PROCEDURE `getNormativesForTest1` (IN `gender` TEXT)   BEGIN
...
CREATE DEFINER=`root`@`%` PROCEDURE `getNormativesForTest2` (IN `gender` TEXT)   BEGIN
...
CREATE DEFINER=`root`@`%` PROCEDURE `getNormativesForTest3` (IN `gender` TEXT)   BEGIN
